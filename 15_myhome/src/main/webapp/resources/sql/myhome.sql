...
CREATE TABLE FREE_T (
    ...
    GROUP_NO    NUMBER              NOT NULL,  -- 원글과 모든 댓글(댓글, 대댓글)은 동일한 GROUP_NO를 가져야 한다.
...
INSERT INTO FREE_T VALUES (FREE_SEQ.NEXTVAL, 'user1@naver.com', '내용11', SYSTIMESTAMP, 1, 0, FREE_SEQ.CURRVAL, 0);
...
INSERT INTO FREE_T VALUES (FREE_SEQ.NEXTVAL, 'user1@naver.com', '내용16', SYSTIMESTAMP, 1, 0, FREE_SEQ.CURRVAL, 0);
...
-- 2) 블로그 상세 정보 조회
SELECT B.BLOG_NO, B.TITLE, B.CONTENTS, B.IP, B.CREATED_AT, B.MODIFIED_AT, U.USER_NO, U.EMAIL, U.NAME
  FROM USER_T U, BLOG_T B
 WHERE U.USER_NO = B.BLOG_NO
   AND B.BLOG_NO = 1;

-- 3) 댓글 목록
-- 블로그 - 댓글 : 1:N (댓글이 달린 블로그 정보는 이미 상세보기에 모두 표시되어 있으므로 여기에선 조인할 필요가 없음)
-- 사용자 - 댓글 : 1:N (댓글의 사용자 이름을 표시)
SELECT ROW_NUMBER() OVER(ORDER BY GROUP_NO DESC, DEPTH ASC, COMMENT_NO DESC) AS RN, C.COMMENT_NO, C.CONTENTS, C.BLOG_NO, C.CREATED_AT, C.STATUS, C.DEPTH, C.GROUP_NO, U.USER_NO, U.NAME
