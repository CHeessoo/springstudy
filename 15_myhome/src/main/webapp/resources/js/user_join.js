...
const fnCheckEmail = () => {
  $('#btn_get_code').click(() => {
    ...
    new Promise((resolve, reject) => {
      ...
      if(!regEmail.test(email)){ // test() 메소드 : 검사가 성공하면 true, 실패하면 false
        reject(1);
        return;
      }
      // 2. 이메일 중복 체크
      ...
      $.ajax({
        ...
        success: (resData) => {  // resData === {"code": "6자리코드"}
          ...
          $('#btn_verify_code').click(() => {
            emailPassed = $('#code').val() === resData.code; // 이메일 인증 성공 시
            if(emailPassed){
              alert('이메일이 인증되었습니다.');
            } else {
...
const fnCheckPassword = () => {
  $('#pw').keyup((ev) => {
    ...
    pwPassed = pw.length >= 8 && pw.length <= 20 && validPwCount >= 2; // true 또는 false 값을 전역변수 상태로 만들어줌.
    if(pwPassed){
...
const fnCheckPassword2 = () => {
  $('#pw2').blur((ev) => {  // blur 이벤트는 입력을 끝내고 빠져나갈 때 딱 한번 동작한다.
    let pw = $('#pw').val();
