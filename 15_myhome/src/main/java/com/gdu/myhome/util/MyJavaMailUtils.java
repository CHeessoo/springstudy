...
@PropertySource(value="classpath:email.properties")  // property파일의 내용 읽어오기
@Component
public class MyJavaMailUtils {
  
  @Autowired
  private Environment env; // property source와 함께 동작하는 Spring Environment 코드
  ...
  public void sendJavaMail(String to, String subject, String content) {
    ...
    try {
      ...
      properties.put("mail.smtp.host", env.getProperty("spring.mail.host")); // (실제로 API에게 전달하는 Key 이름, 전달할 프로퍼티 이름)
      ...
      Session session = Session.getInstance(properties, new Authenticator() {
        ...
        protected PasswordAuthentication getPasswordAuthentication() {
          return new PasswordAuthentication(env.getProperty("spring.mail.username"), env.getProperty("spring.mail.password")); // user name, password
        }
      });
      
      // 메일 만들기 (보내는 사람 + 받는 사람 + 제목 + 내용)
      MimeMessage mimeMessage = new MimeMessage(session);
      mimeMessage.setFrom(new InternetAddress(env.getProperty("spring.mail.username"), "사이트관리자"));  // 보내는 사람 
      mimeMessage.setRecipient(Message.RecipientType.TO, new InternetAddress(to));                        // 받는 사람
      mimeMessage.setSubject(subject);                                                                    // 제목
      mimeMessage.setContent(content, "text/html; charset=UTF-8");                                        // 내용, 타입
